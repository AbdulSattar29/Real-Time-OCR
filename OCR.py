...
def put_ocr_boxes(boxes, frame, height, crop_width=0, crop_height=0, view_mode=1):
    """
    Draws text bounding boxes at tesseract-specified text location. Prints detected text on the console, translates it,
    and displays the translated text.
    ...
    :param view_mode: View mode to specify the style of bounding box
    ...
    if boxes is not None:  # Defends against empty data from tesseract image_to_data
        ...
        if text:  # Only process if there's valid text
            ...
            if translated_text.isascii():  # CV2 can only display ASCII characters
                cv2.putText(frame, translated_text, (5, height - 10), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 255, 255))
...
def ocr_stream(crop: List[ int], source: int = 0, view_mode: int = 1, language=None):
    """
